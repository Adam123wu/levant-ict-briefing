...
const issue = "W37-38";
const generated = "2026-09-13";
const periodZh = "2026年9月5日 — 9月13日（滚动更新，完整周期至9月18日）";
const periodEn = "5–13 September 2026 (rolling update; the full W37–38 window closes on 18 September)";
...
const countries = {
  iq: {
    ...
    sections: [
      ...
      [
        item(
          "2026-09-12", "总理府 Telegram + Shafaq 交叉核验",
          "北方石油公司签署阿吉勒油田 25 年开发运营合同，数字油田与工业网络进入执行前需求识别期",
          "总理府确认北方石油公司与伊拉克私营开发商 KEPPT 签署阿吉勒油田开发运营合同。合同目标是把天然气日产量从约 1.35 亿标准立方英尺提升至 3 亿，并把原油日产量从约 3 万桶提升至 4 万桶。主开发合同已经签署，但公开信息没有披露 SCADA、工业网络、数据中心或网络安全分包范围。",
          "列为 L2 油气数字化机会：向北方石油公司和主承包商核实 OT/SCADA、生产数据采集、园区光纤、视频监控、边缘计算与网络安全包；报价前完成 KEPPT、分包商、银行和最终受益人筛查，不得把油田主合同误写成 ICT 已授标。",
          [["总理府 Telegram 9/12", "https://t.me/IraqiPmo/65875"], ["Shafaq 9/12", "https://api.shafaq.com/en/Economy/Ajeel-field-targets-122-increase-in-gas-production"]],
          "North Oil Company signs a 25-year Ajeel field development and operating contract, opening an execution-stage discovery window for digital-oilfield and industrial-network needs",
          "The PMO confirmed that North Oil Company and Iraqi private developer KEPPT signed the Ajeel field development and operating contract. It targets an increase in gas production from about 135 million to 300 million standard cubic feet per day and in crude output from about 30,000 to 40,000 barrels per day. The main development contract is signed, but no public source discloses SCADA, industrial-network, data-centre or cybersecurity subpackages.",
          "Treat this as an L2 oil-and-gas digitalisation opportunity. Confirm OT/SCADA, production-data acquisition, site fibre, video, edge-compute and cybersecurity packages with North Oil Company and the main contractor. Screen KEPPT, subcontractors, banks and beneficial owners before quotation, and do not misstate the field contract as an ICT award."
        ),
        item(
          "2026-09-12", "本期综合研判",
          "本期四条优先机会：国际过境网络、主权 AI 基础设施、政府安全 LTE/5G、阿吉勒数字油田配套",
          "四条机会均已出现官方需求信号，但成熟度不同：Ooredoo/ITPC 过境合同已签、属于 L1；政府安全 LTE 扩围和国家 AI 基础设施属于 L2；阿吉勒油田主开发合同已签，但 ICT 分包范围尚未披露，因此列为 L2 需求识别。Korek 相关机会因监管执行和回款风险降为观察或暂停。",
          "七日动作：向 ITPC 提交过境骨干假设清单；向通信部预约政务专网技术会；向 CMC 提交主权 AI 参考架构；向北方石油公司/KEPPT 核实 OT 与网络分包；对上述项目伙伴执行 OFAC 增量筛查。",
          ...
          "Four priority opportunities this period: international transit, sovereign AI infrastructure, secure government LTE/5G and Ajeel digital-oilfield follow-ons",
          "All four now have official demand signals, but maturity differs: the Ooredoo/ITPC transit contract is signed and rated L1; secure government LTE expansion and national AI infrastructure are L2; the Ajeel field's main development contract is signed, but its ICT subpackage is undisclosed and therefore remains an L2 discovery item. Korek-related opportunities move to watch or hold because of enforcement and collection risk.",
          "Seven-day actions: submit a transit-backbone assumptions list to ITPC; request a government-network technical session with the ministry; give CMC a sovereign-AI reference architecture; confirm OT and network packages with North Oil Company/KEPPT; and run incremental OFAC screening on all project partners."
        )
      ],
      [
...
const summaryZh = [
  ...
  "🇮🇶 北方石油公司签署阿吉勒油田 25 年开发运营合同；数字油田、OT/SCADA 和工业网络仅列 L2，尚无 ICT 分包授标证据。",
  ...
  "📌 本期为 W37–38 滚动更新，信息截止 2026-09-13；9 月 18 日前的新事实将在后续自动任务继续补入同一期。"
];
const summaryEn = [
  ...
  "North Oil Company signed a 25-year Ajeel field development and operating contract; digital-oilfield, OT/SCADA and industrial-network follow-ons are L2 signals, with no evidence yet of an ICT subcontract award.",
  ...
  "This is a rolling W37–38 update through 13 September 2026. New verified facts through 18 September will be added to the same issue by subsequent automation runs."
];
...
</style></head><body><main class="wrap"><section class="hero"><h1>黎凡特 ICT 双周简报 · ${issue}</h1><div class="top-meta">生成日期：${generated} · 作者：${author}</div><div class="retro-date">${periodZh}</div><div class="kpis"><div class="kpi"><div class="kpi-val">${totalNews}</div><div class="kpi-lbl">本期新闻</div></div><div class="kpi"><div class="kpi-val">${opportunities}</div><div class="kpi-lbl">商机信号</div></div></div><ul class="ov-list">${summaryZh.map((line) => `<li>${esc(line)}</li>`).join("")}</ul></section>${countryHtml}<div class="foot">公开信源研判，不构成法律意见 · ${author}</div></main></body></html>`;
